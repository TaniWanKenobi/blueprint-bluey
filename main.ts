...
async function createTicket(
  ...
  try {
    ...
    try {
      ...
      aiResponse =
        "I couldn't generate a response. Please try again or contact a staff member directly.";
    }
    ...
    const result = await client.chat.postMessage({
      ...
      channel: TICKETS_CHANNEL,
      blocks: createTicketBlocks(aiResponse, message.channel, message.ts),
    });
...
async function updateTicketMessage(
  ...
  try {
    ...
    if (ticket.claimers.length > 0) {
      headerText = `Claimed by: ${ticket.claimers
        .map((id) => `<@${id}>`)
        .join(", ")}`;
    } else if (ticket.notSure.length > 0) {
      headerText = `Not Claimed | Not sure: ${ticket.notSure
        .map((id) => `<@${id}>`)
        .join(", ")}`;
    }
...
async function resolveTicket(
  ...
  try {
    ...
    try {
      ...
      if (
        !originalMessageCheck.ok ||
        !originalMessageCheck.messages ||
        originalMessageCheck.messages.length === 0
      ) {
        logger.warn(
          `Original message for ticket ${ticketTs} no longer exists or is inaccessible. Proceeding with ticket resolution.`
        );
      } else {
      ...
      logger.warn(
        `Failed to check original message for ticket ${ticketTs}:`,
        error
      );
    ...
    const existingEntryIndex = newEntry.findIndex(
      (e) => e.slack_id === resolver
    );
...
app.event("message", async ({ event, client, logger }) => {
  ...
  if (ticket) {
    ...
    const success = await claimTicket(
      threadReply.user,
...
app.action("mark_resolved", async ({ body, ack, client, logger }) => {
  ...
  if (!isTicketChannelMember(userId)) {
    logger.info(
      `User ${userId} tried to resolve a ticket but is not in the tickets channel`
    );
    return;
  }
...
app.action("not_sure", async ({ body, ack, client, logger }) => {
  ...
  if (!isTicketChannelMember(userId)) {
    logger.info(
      `User ${userId} tried to mark "not sure" but is not in the tickets channel`
    );
    return;
  }
...
app.action("assign_user", async ({ body, ack, client, logger }) => {
  ...
  if (!isTicketChannelMember(userId)) {
    logger.info(
      `User ${userId} tried to assign a ticket but is not in the tickets channel`
    );
    return;
  }
...
app.action("show_ai_response", async ({ body, ack, client, logger }) => {
  ...
  if (!isTicketChannelMember(userId)) {
    logger.info(
      `User ${userId} tried to show AI response but is not in the tickets channel`
    );
    return;
  }
...
app.action("hide_ai_response", async ({ body, ack, client, logger }) => {
  ...
  if (!isTicketChannelMember(userId)) {
    logger.info(
      `User ${userId} tried to hide AI response but is not in the tickets channel`
    );
    return;
  }
...
async function fetchAIResponse(userInput) {
  ...
  try {
    ...
    const text =
      data.choices?.[0]?.message?.content || "Error: No response content";
    console.log(
      "AI Response:",
      text.replace(/<think>[\s\S]*<\/think>/gm, "").trim()
    );
    return text.replace(/<think>[\s\S]*<\/think>/gm, "").trim();
  } catch (error) {
